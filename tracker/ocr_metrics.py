...
# Ported from the Bazaar Chronicle project (MIT licensed). This reads the
# numeric run-result overlay (wins/gold/prestige/level/income/max_health)
# from a screenshot of the final board -- those numbers are never written
# to Player.log, so OCR stays the only source for them.
...
def _configure_tesseract() -> bool:
    ...
    bundled_exe = base / "third_party" / "tesseract" / "tesseract.exe"
    bundled_tessdata = base / "third_party" / "tesseract" / "tessdata"
...
def _prep_for_tesseract(pil_img: Image.Image, *, scale: int = 3, dilate: bool = True) -> Image.Image:
    import numpy as np
    import cv2
...
def _try_read_wins_int(pil_crop: Image.Image) -> tuple[int | None, dict]:
    ...
    variants = [("orig", pil_crop), ("isolated", isolated)]
    ...
    for label, img in variants:
        ...
        for psm in (10, 8, 7):
            ...
            row = {"mode": label, "psm": psm, "raw": raw, "value": val}
            attempts.append(row)

            if val is not None:
                best_val = val
                best_row = row
                return best_val, {"isolation": iso_dbg, "best": best_row, "attempts": attempts}

    return None, {"isolation": iso_dbg, "best": best_row, "attempts": attempts}
...
def _try_read_oneish_int(pil_crop: Image.Image) -> tuple[int | None, dict]:
    ...
    for label, img in variants:
        ...
        for psm in (7, 8, 10):
            raw = pytesseract.image_to_string(
                prep, config="--oem 1 --psm %d -c tessedit_char_whitelist=1Il|" % psm
            ).strip()
            ...
            if val is not None:
                return val, {"isolation": iso_dbg, "best": row, "attempts": attempts}
...
def _prep_hsv_whitecore(pil_img: Image.Image, *, scale: int = 14) -> Image.Image:
    ...
    mask = ((s < 90) & (v > 150)).astype("uint8") * 255
    img = 255 - mask
...
def _try_read_int(pil_crop: Image.Image) -> tuple[int | None, dict]:
    ...
    def _run_variants(img: Image.Image, label_prefix: str) -> list[tuple[int, dict]]:
        ...
        for mode, prep_cfg in variants:
            ...
            for psm, oem in ((7, 1), (8, 1), (7, 3)):
                ...
                row = {
                    "mode": mode, "psm": psm, "oem": oem,
                    "scale": prep_cfg["scale"], "dilate": prep_cfg["dilate"],
                    "raw": raw, "value": val,
                }
                attempts.append(row)
    ...
    if not candidates:
        try:
            ...
            for kind, cfg in (
                ...
                row = {
                    "mode": kind, "psm": int(kind[-1]), "oem": 1,
                    "scale": 14, "dilate": False, "raw": raw, "value": val,
                }
                attempts.append(row)
    ...
    return best_val, {
        "isolation": iso_dbg, "best": best_row,
        "vote_counts": dict(counts), "attempts": attempts,
    }


def _find_matching_resolution_key(
    width: int, height: int, rois_for_resolution: Dict[str, Any], tolerance: int = 10
) -> str | None:
...
def extract_run_metrics(
    ...
    if key is None:
        raise RuntimeError(f"No OCR ROIs for resolution {actual_key}. Add it to tracker/ocr_rois.py")
    ...
    for field, roi in rois.items():
        ...
        if field == "wins" and val is None:
            ...
            if val2 is not None:
                val = val2
                dbg = {"fallback": "wins_specialized", "generic": dbg, "wins_specialized": dbg2}

        if field != "wins" and val is None:
            val3, dbg3 = _try_read_oneish_int(crop)
            if val3 is not None:
                val = val3
                dbg = {"fallback": "oneish_specialized", "generic": dbg, "oneish_specialized": dbg3}
